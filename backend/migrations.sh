...
for i in {1..10}; do
    ...
    if PGPASSWORD="$POSTGRES_DB_PASSWORD" psql "host=$POSTGRES_HOST port=$POSTGRES_PORT dbname=$POSTGRES_DATABASE user=$POSTGRES_DB_USERNAME" -c '\q'; then
        break
    fi
...
PGPASSWORD="$POSTGRES_ADMIN_PASSWORD" psql "host=$POSTGRES_HOST port=$POSTGRES_PORT dbname=$POSTGRES_DATABASE user=$POSTGRES_ADMIN_USERNAME" -c "CREATE SCHEMA IF NOT EXISTS $POSTGRES_DB_SCHEMA AUTHORIZATION \"$POSTGRES_DB_USERNAME\""
...
if PGPASSWORD="$POSTGRES_ADMIN_PASSWORD" psql -h "$POSTGRES_HOST" -d "$POSTGRES_DATABASE" -U "$POSTGRES_ADMIN_USERNAME" -tAc "SELECT 1 FROM pg_roles WHERE rolname='bcgw'" | grep -q 1; then
    echo "bcgw user exists. Granting permissions..."
    PGPASSWORD="$POSTGRES_ADMIN_PASSWORD" psql -h "$POSTGRES_HOST" -d "$POSTGRES_DATABASE" -U "$POSTGRES_ADMIN_USERNAME" <<EOF
...
if [ "$SEED_ENABLED" != "true" ]; then
    ...
    if [ -z "$SEED_DATA_PATH" ]; then
        ...
        if [ "$(PGPASSWORD="$POSTGRES_ADMIN_PASSWORD" psql -h "$POSTGRES_HOST" -d "$POSTGRES_DATABASE" -U "$POSTGRES_ADMIN_USERNAME" -t -c "SELECT count(*) FROM sites.sites;")" -gt 0 ]; then
