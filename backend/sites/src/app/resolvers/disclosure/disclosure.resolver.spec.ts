...
describe('DisclosureResolver', () => {
    ...
    beforeEach(async () => {
        const module = await Test.createTestingModule({
            providers: [
                ...
                },
                {
                    ...
                    useValue: {
                        createResponse: jest.fn((message: string, httpStatusCode: number, success: boolean, data?: SiteProfiles[]) => ({
                            message,
                            httpStatusCode,
                            success,
                            data,
        ...
        genericResponseProvider = module.get<GenericResponseProvider<SiteProfiles[]>>(GenericResponseProvider);
    });
    ...
    describe('getSiteDisclosureBySiteId', () => {
        it('should return site disclosure when data is found', async () => {
            ...
            jest.spyOn(disclosureService, 'getSiteDisclosureBySiteId').mockResolvedValueOnce(mockSiteProfile);

            const result = await resolver.getSiteDisclosureBySiteId(siteId);

            expect(result).toEqual(expectedResponse);
            expect(genericResponseProvider.createResponse).toHaveBeenCalledWith('Site Disclosure fetched successfully', 200, true, mockSiteProfile);
        });
        ...
        it('should return 404 error when no data is found', async () => {
            ...
            jest.spyOn(disclosureService, 'getSiteDisclosureBySiteId').mockResolvedValueOnce([]);

            const result = await resolver.getSiteDisclosureBySiteId(siteId);

            expect(result).toEqual(expectedResponse);
            expect(genericResponseProvider.createResponse).toHaveBeenCalledWith(`Site Disclosure data not found for site id: ${siteId}`, 404, false, null);
        });
    });
});
