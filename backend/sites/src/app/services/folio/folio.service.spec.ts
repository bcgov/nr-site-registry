...
describe('FolioSerivce', () => {
    ...
    const folios : Folio[] =
    [
      { folioId : "1" , id : 1 , userId : "1" , description : "test" , whoCreated : "test", whenCreated : new Date("07-26-2024"), whoUpdated : "test" , whenUpdated :  new Date("07-26-2024"), folioContents: null}
    ]
    ...
    beforeEach(async ()=>{
        ...
        folioContentRepository = module.get<Repository<FolioContents>>(getRepositoryToken(FolioContents));
    })
    ...
    afterEach(() => {
      ...
      it('Should return sites for folio', async ()=>{
        ...
        jest.spyOn(folioContentRepository, 'find').mockResolvedValue(
          folioContent
        )

        const result = await folioSerivce.getSitesForFolio({id:1, userId: ""}, user);
      ...
      it('Should Not Add A New Folio', async ()=>{
        ...
        jest.spyOn(folioRepository, 'save').mockResolvedValue(
         folios[0]
        )

        const result = await folioSerivce.addFolio({id:2, userId: "", folioId: "1", description: "", whoCreated: "midhun", whenUpdated: "26-07-2024"}, user);
      ...
      it('Should Update Folio', async ()=>{
        ...
        const result = await folioSerivce.updateFolio([{id:2, userId: "", folioId: "1", description: ""}], user);
      ...
      it('Should Add Site To Folio', async ()=>{
        ...
        jest.spyOn(folioRepository, 'findOne').mockResolvedValue(
         ...
         jest.spyOn(folioContentRepository, 'findOne').mockResolvedValue(
         null
        )

        jest.spyOn(folioContentRepository, 'save').mockResolvedValue(
         folioContent[0]
        )
         
        const result = await folioSerivce.addSiteToFolio( [{id: 1, siteId: "5" , folioId: "1", userId:"1", whoCreated: ""}] , user);
