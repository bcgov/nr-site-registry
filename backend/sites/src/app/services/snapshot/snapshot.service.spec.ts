...
describe('SnapshotService', () => {
    ...
    beforeEach(async () => {
        ...
        snapshotRepository = module.get<Repository<Snapshots>>(getRepositoryToken(Snapshots));
      });
    ...
    describe('getSnapshotsByUserId', () => {
        it('should return an array of snapshots for a given userId', async () => {
            ...
            expect(snapshotRepository.find).toHaveBeenCalledWith({ where: { userId } });
        });
    ...
    describe('getSnapshotsById', () => {
        ...
        it('should throw an error if repository throws an error', async () => {
            ...
            await expect(service.getSnapshotsById(id)).rejects.toThrow('Database connection error');
            expect(snapshotRepository.find).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe('createSnapshot', () => {
        it('should create a snapshot successfully', async () => {
            const snapshotDto: SnapshotDto = {
               ...
               // Mock recentViewsRepository methods
                ...
                jest.spyOn(snapshotRepository, 'save').mockResolvedValueOnce({} as Snapshots); // Mock save method
            ...
            expect(snapshotRepository.save).toHaveBeenCalledWith(expect.objectContaining(snapshotDto));
        });
        ...
        it('should throw an error if repository throws an error', async () => {
            ...
            await expect(service.createSnapshot(snapshotDto)).rejects.toThrow('Failed to insert snapshot.');
            expect(snapshotRepository.save).toHaveBeenCalledTimes(1);
        });
    });
});