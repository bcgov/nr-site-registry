...
export class SiteService {
  ...
  async searchSites(
    ...
    if (id) query.andWhere('sites.id = :id', { id });
    if (srStatus) query.andWhere('sites.srStatus = :srStatus', { srStatus });
    if (siteRiskCode)
      query.andWhere('sites.site_risk_code = :siteRiskCode', { siteRiskCode });
    if (commonName)
      query.andWhere('sites.common_name = :commonName', { commonName });
