    ...
    <div >
      ...
      <div className="d-flex align-items-center justify-content-center w-100 position-relative">
        ...
        {!createMode && searchTerm.length <= 0 ? (
          <span id="search-icon" className="search-icon custom-icon position-absolute px-2">
            <MagnifyingGlassIcon />
          </span>
        ) : (
     ...
     { createMode &&  createNewLabel &&  <div className="search-add-new" onClick={()=>{ addNewHandler(searchTerm)}}>Add {createNewLabel}</div>}
     </div>
    </div>
  );
};
