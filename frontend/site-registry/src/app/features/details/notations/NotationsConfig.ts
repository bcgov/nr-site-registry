...
import { notationClassDrpdown, notationParticipantRoleDrpdown, participantNameDrpdown} from "../dropdowns/DropdownSlice";

export const GetNotationConfig = () => {
    const participantName = useSelector(participantNameDrpdown);
    const ministryContact = participantName.data.filter((items: any) => items.metaData === 'EMP').flatMap((items: any) => items.dropdownDto);
