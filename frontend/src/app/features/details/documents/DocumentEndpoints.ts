...
export const createChildBucket = async (parentBucketId: string) => {
  try {
    ...
    const response = await getAxiosInstance(COMS_API).put(
      `${COMS_ENDPOINTS.BUCKET}/${parentBucketId}${COMS_ENDPOINTS.CHILD}`,
      params,
    );
    ...
    console.error(`Error: Uploading Document-createChildBucket: ${error}`);
    throw error;
  }
};
...
export const updateObject = async (objectId: string, file: any) => {
  try {
    const config = {
      headers: {
        'Content-Disposition': setDispositionHeader(file.name),
...
const getObjectVersions = async (objectId: string) => {
  try {
    const GET_VERSIONS_ENDPOINT = `${COMS_ENDPOINTS.OBJECT}/${objectId}${COMS_ENDPOINTS.VERSION}`;
    const response = await getAxiosInstance(COMS_API).get(
    ...
    console.error('Error in getting object version details:', error);
    throw error;
  }
};

export const permanentObjectDeletion = async (objectId: string) => {
  try {
    //const URL = 'http://localhost:4050/api/v1/';
    // Get all versions of the object
    const res = await getObjectVersions(objectId);

    // Check if there's data
    if (res?.data?.length > 0) {
      // Prepare an array of delete promises
      const deletePromises = res?.data?.map(async (object: any) => {
        try {
          // Delete each object by its objectId and s3VersionId
          const comsDeleteObj = await deleteObject(
            object?.objectId,
            object?.s3VersionId,
          );
        } catch (deleteError) {
          console.error(
            `Error deleting object ${object?.objectId}:`,
            deleteError,
          );
        }
      });
    ...
    console.error('Error in deleting objects:', error);
  }
};
