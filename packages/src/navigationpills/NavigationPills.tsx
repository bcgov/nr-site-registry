...
import { Link, useLocation } from 'react-router-dom'; // Importing React Router for navigation and location tracking
...
const NavigationPills: React.FC<INavigationPills> = ({
  ...
  customNavPillsCss, // Optional custom CSS class for navigation pills
  customMobileNavPillsCss, // Optional custom CSS for mobile view of pills
      ...
      <div className="d-flex d-xxl-flex d-xl-flex gap-2 d-none">
        ...
        {items.map((item, index) => (
          <Link
            key={index}
            to="#"
            className={`${customNavPillsCss ? customNavPillsCss : `d-flex gap-2 custom-nav-pill ${isDisable ? 'disabled default' : index === activeIndex ? 'active' : 'enabled'}`}`}
            // If pills are not disabled, handle the click event and set the active index
            onClick={isDisable ? () => {} : () => handlePillClick(index)}
          >
            {item} {/* Render the pill text */}
          </Link>
        ))}
      </div>
        ...
        <div className="d-flex justify-content-between w-100 flex-column flex-sm-row">
          <div>
            ...
            <Actions
              ...
              onItemClick={isDisable ? () => {} : (_, index) => handlePillClick(index)} // Handle dropdown item click
            ...
            <div className="d-flex align-items-center">
              ...
              <div className="ps-3 pe-2 m-0 p-0 w-100 text-center">
                ...
                {items.map((item, index) => (
                  <div
                    key={index}
                    className={`${customMobileNavPillsCss ? customMobileNavPillsCss : `custom-nav-pill ${index === activeIndex ? 'd-block active' : 'd-none'}`}`}
                  >
                    {item} {/* Show the pill text */}
                  </div>
                ))}
              </div>
