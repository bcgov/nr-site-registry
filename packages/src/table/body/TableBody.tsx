...
const TableBody: FC<TableBodyProps> = ({
  ...
  useEffect(() => {
    ...
    const rowsIds: string[] = data.map((_: any, index: number) => {
        ...
        <td colSpan={20} className="noContent table-border-light">
          {isLoading === RequestStatus.loading ? (
            <div className="content-loading">
              <FaSpinner data-testid="loading-spinner" className="fa-spin " />
  ...
  return (
    <tbody>
      {data.length === 0
        ...
        : data.map((_: any, index: number) => renderTableRow(index))}
    </tbody>
  );
};
